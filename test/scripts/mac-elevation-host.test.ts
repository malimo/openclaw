...
function createArtifactVerificationHarness() {
  ...
  writeExecutable(
    ...
    [
      ...
      '  if [[ -f "$target/Contents/MacOS/OpenClaw" ]] && head -n 1 "$target/Contents/MacOS/OpenClaw" | grep -q \'^#!\'; then',
...
function createInstallRollbackHarness(
  ...
  writeExecutable(
    ...
    [
      "#!/usr/bin/env bash",
      "set -euo pipefail",
      'if [[ "$TEST_SIGNAL_DURING_CUSTODY" == "1" && "${2:-}" == *.custody.* ]]; then',
      '  /bin/mv "$@"',
      '  kill -TERM "$PPID"',
      "  exit 0",
      "fi",
      ...
      "  exit 0",
      "fi",
      'if [[ "$TEST_SIGNAL_BEFORE_ROLLBACK_APP_MOVE" == "1" && "${2:-}" == *.rollback-elevation-host-* ]]; then',
      '  kill -TERM "$PPID"',
      "  exit 7",
      "fi",
      'exec /bin/mv "$@"',
